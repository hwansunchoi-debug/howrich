-- 새로운 카테고리 추가 (기존것과 중복되지 않는 것들만)
INSERT INTO categories (name, type, color, icon, user_id) 
SELECT '취미&여가', 'expense', '#8b5cf6', 'gamepad-2', null
WHERE NOT EXISTS (SELECT 1 FROM categories WHERE name = '취미&여가');

INSERT INTO categories (name, type, color, icon, user_id)
SELECT '편의점&마트&잡화', 'expense', '#10b981', 'shopping-cart', null  
WHERE NOT EXISTS (SELECT 1 FROM categories WHERE name = '편의점&마트&잡화');

INSERT INTO categories (name, type, color, icon, user_id)
SELECT '카페&간식', 'expense', '#f59e0b', 'coffee', null
WHERE NOT EXISTS (SELECT 1 FROM categories WHERE name = '카페&간식');
