...
-- Create new policies for merchant mappings - anyone can read, but only authenticated users can create/update
CREATE POLICY "Anyone can view merchant mappings" 
ON merchant_category_mappings 
FOR SELECT 
USING (true);

CREATE POLICY "Authenticated users can create merchant mappings" 
ON merchant_category_mappings 
FOR INSERT 
