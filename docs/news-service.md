...
이슈가 오래 이어져도 타임라인이 끊기지 않도록, 이슈에 묶인 기사는 지우지 않는다.

| 대상 | 보관 |
| --- | --- |
| 이슈에 묶인 기사 | 제목·언론사·링크·보도시각을 계속 보관 |
| 위 기사의 요약(summary) | 7일 뒤 비운다. 용량의 대부분을 차지하고 이슈를 묶을 때만 쓴다 |
| 이슈에 묶이지 않은 기사 | 7일 뒤 삭제. 전체의 절반 이상이다 |
| 시간대별 요약(timeline_events) | 계속 보관. 기사보다 훨씬 작다 |

무료 플랜 500MB 기준으로 하루 1,500건을 수집해도 몇 년치를 담을 수 있다.
현재 사용량은 `select * from news_storage_usage;` 로 확인한다.

수집할 때는 서버가 준 ETag / Last-Modified 를 기억했다가 다음 요청에 함께
보낸다. 바뀐 내용이 없으면 본문 없이 304 만 받으므로 같은 기사를 반복해서
내려받지 않는다. 저장 단계에서도 url 이 unique 라 같은 기사가 두 번
저장되지 않는다.
