...
기존 가계부 앱과 같은 프로젝트 안에 있고, 화면 경로만 분리되어 있다.

| 경로 | 화면 | 로그인 |
| --- | --- | --- |
| `/news` | 메인: 현재 주요 이슈 목록 | 불필요 |
| `/news/issue/:issueId` | 이슈 상세: 현재 상황 + 기사 타임라인 | 불필요 |
